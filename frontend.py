...
if "topic_prefill" in st.session_state and isinstance(
    st.session_state["topic_prefill"], str
):
...
if out:
    ...
    with tab_evidence:
        ...
        if not evidence:
            st.info(
                "No evidence returned (maybe closed_book mode or no Tavily key/results)."
            )
        else:
            rows = []
            for e in evidence:
